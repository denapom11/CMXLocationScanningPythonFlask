(function ($) {
  ...
  function track(client) {
    clearAll();
    if (client !== null && client.geoCoordinate !== null && !(typeof client.geoCoordinate === 'undefined')) {
      ...
      if (client.mapInfo.mapHierarchyString != null && client.mapInfo.mapHierarchyString  !== "") {
        floorStr = " at '" + client.mapInfo.mapHierarchyString  + "'"
      } else {
        floorStr = "";
      }
      $('#last-mac').text(mfrStr + "'" + lastMac + "'" + osStr + ssidStr +
        " last seen on " + client.statistics.lastLocatedTimeseen + floorStr +
        " with uncertainty " + client.confidenceFactor.toFixed(1) + " meters (reloading every 20 seconds)");
      map.setCenter(pos);
      ...
      clientUncertaintyCircle = new google.maps.Circle({
        map: map,
        center: pos,
        radius: client.confidenceFactor,
        fillColor: 'RoyalBlue',
        fillOpacity: 0.25,
  ...
  function addMarker(client) {
    if (client !== null && client.geoCoordinate !== null && !(typeof client.geoCoordinate === 'undefined')){
      ...
      google.maps.event.addListener(m, 'click', function () {
        infoWindow.setContent("<div>" + client.macAddress + "</div> (<a class='client-filter' href='#' data-mac='" +
          client.macAddress + "'>Follow this client)</a>");
        infoWindow.open(map, m);
      });
      if (client.macAddress === lastInfoWindowMac) {
        infoWindow.open(map, m);
      }
      var pos = new google.maps.LatLng(client.geoCoordinate.latitude, client.geoCoordinate.longitude);
      map.setCenter(pos);
  ...
  function startLookupAll() {
    if (lastEvent !== null) { window.clearInterval(lastEvent); }
    lastEvent = window.setInterval(lookupAll, 10000);
    lookupAll();
  }
